...
namespace MM.Pages.Client.Admin
{
    ...
    public class FinanceModel : PageModel
    {
        ...
        private string EntityName = "Pastel Integration - Individual";
        ...
        public FinanceModel(
           SignInManager<ApplicationUser> signInManager,
            ILogger<FinanceModel> logger, IActivity activity, IWebHostEnvironment env, RoleManager<ApplicationRole> roleManager, UserManager<ApplicationUser> userManager,
        ...
        public async Task<FileContentResult> OnGetExportAsync()
        {
            ...
            var memberUsers = await _context.MemberUser.Include(x => x.MemberStatus).Where(x => x.MemberStatus.Name == "Active"||(x.AccountBalanceOftheCompany != null && x.AccountBalanceOftheCompany != 0) ).Select(p => new { p.MemberCode, AccountBalance = p.AccountBalanceOftheCompany == null ? 0.00M : p.AccountBalanceOftheCompany }).ToListAsync();


            notification = new Notification { message = "Account balances downloaded successfully", notificationtype = "success" };
            return File(ExcelHelper.ExportExcel(memberUsers, false, displayColumns, columnsToRemove, ""), GlobalVariables.ExcelFileContenttType, "CRM Individual Balances " + HelperMethods.GetDateStamp() + ".xlsx");
        }
        ...
        public async Task<FileContentResult> OnGetExportDetailAsync()
        {
            ...
            string[] columnsToRemove = { };
            var memberUsers = await _context.MemberUser.Include(x=>x.ApplicationUser).Include(x => x.MemberStatus).
                Where(x => x.MemberStatus.Name == "Active" || (x.AccountBalanceOftheCompany != null && x.AccountBalanceOftheCompany != 0)).Select(p => new { p.MemberCode, 
                AccountBalance = p.AccountBalanceOftheCompany == null ? 0.00M : p.AccountBalanceOftheCompany,
                Status = p.MemberStatus == null ? "" : p.MemberStatus.Name,Name= p.ApplicationUser.FullName
                }).ToListAsync();


            //var memberUsers = await _context.MemberUser.Include(x => x.ApplicationUser).Include(x => x.MemberStatus).Where(x => x.MemberStatus.Name == "Active" || (x.AccountBalanceOftheCompany != null && x.AccountBalanceOftheCompany != 0)).
            //Select(p => new {
            //    p.MemberCode,
            //    AccountBalance = p.AccountBalanceOftheCompany == null ? 0.00M : p.AccountBalanceOftheCompany,
            ...
            notification = new Notification { message = "Account balances downloaded successfully", notificationtype = "success" };
            return File(ExcelHelper.ExportExcel(memberUsers, false, displayColumns, columnsToRemove, ""), GlobalVariables.ExcelFileContenttType, "Detailed CRM Individual Balances" + HelperMethods.GetDateStamp() + ".xlsx");
        }
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            var Data = ExcelHelper.GetDataFromExcel(Path.Combine(_env.ContentRootPath + "\\" + _configuration["StoredFilesPath"], "Finance", "Finance.xlsx"));
            string membercode = string.Empty;
            int i = 1;
            int errorcount = 0;
            int successcount=0;
            var memberUsers = await _context.MemberUser.ToListAsync();
            ...
            foreach (DataRow row in Data.Rows)
            {
                ...
                {
                    var memberUser = memberUsers.Where(x => x.MemberCode == membercode).FirstOrDefault();
                    if (memberUser == null)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            memberUser.AccountBalanceOftheCompany=accountbalanceoftheCompany;
                            _context.MemberUser.Update(memberUser);
                            successcount++;
                        }
        ...
        public async Task UploadFile(IFormFile file)
        {
            ...
            if (file.Length > 0)
            {
                ...
                filePath = Path.Combine(filePath, "Finance.xlsx");
